 ...
 */
class MockDataManager3 : public IDataRepository {
public:
 ...
 */
class MockPersistenceManager3 : public IPersistanceData {
public:
    std::string capturedOutput = "";
...
class MockOutputWriter3 : public IOutputWriter {
public:
    std::string capturedOutput = "";
...
// Tests if PostCommand properly passes the data to the DataRepository.
TEST(PostCommandTest, LocalDataTest) {
    MockDataManager3 mockDataManager;
    MockPersistenceManager3 persistenceManager;
    MockOutputWriter3 mockWriter;
    // -- Start class test --
...
// Tests if PostCommand properly passes the data to the PersistenceData manager.
TEST(PostCommandTest, PersistenceDataTest) {
    MockDataManager3 mockDataManager;
    MockPersistenceManager3 persistenceManager;
    MockOutputWriter3 mockWriter;
    // -- Start class test --
...
// Tests if PostCommand does nothing and returns 404 when the user exists in the system.
TEST(PostCommandTest, UserExistsTest) {
    MockDataManager3 mockDataManager;
    MockPersistenceManager3 persistenceManager;
    MockOutputWriter3 mockWriter;
    // -- Start class test --
    ...
    mockDataManager.user_Exist = true;
